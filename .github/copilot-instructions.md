...
- 站点配置：`hugo.yaml`
  - 关键字段：`baseURL: '/'`（使用相对链接，CI 环境覆盖）、`languageCode: zh-cn`、`title: PaperMod`、`theme: ["PaperMod"]`、`params.*` 按示例站点启用功能（面包屑/目录/复制按钮等）。
  - 链接策略：`relativeURLs: true` + `canonifyURLs: false`，适配 GitHub Pages 与 Netlify 双环境。
- 内容：`content/`（文章与页面应放在这里）。
- 原型/脚手架：`archetypes/default.md`（新建内容时默认 Front Matter：`date`, `draft`, `title`）。
- 产物目录：`public/`（构建输出，勿手改；已在 `.gitignore` 忽略）。
...
---
仍需维护者确认/补充：
- GitHub Pages 仓库类型（用户/组织页 vs 项目页）与自定义域名。如需绝对链接，请提供最终域名以便在 CI 中覆盖 `--baseURL`。
- Netlify 站点绑定的仓库与环境变量（如需）。